...
// Cache for Huddle Settings so we don't re-fetch on every duration change
let _huddleSettingsCache = null;
...
frappe.ui.form.on("Huddle Meeting", {
	refresh(frm) {
		...
		const is_past =
			frm.doc.end_date &&
			frappe.datetime.get_diff(frm.doc.end_date, frappe.datetime.now_datetime()) < 0;
		...
		if (!frm.is_new() && frm.doc.jitsi_url && !is_completed && !is_cancelled && !is_past) {
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method:
							"frappe_huddle.frappe_huddle.doctype.huddle_meeting.huddle_meeting.join_meeting",
			...
			frm.add_custom_button(__("Copy Meeting Link"), function () {
				let frappe_join_url =
					window.location.origin + "/huddle/join?meeting=" + frm.doc.name;
			...
			frm.add_custom_button(
				...
				function () {
					frappe.confirm(
						__("Resend email invites to all participants?"),
						() => {
							frappe.call({
								...
								callback: function (r) {
									if (r.message) {
										frappe.show_alert({
											message: __("Invites queued successfully!"),
											indicator: "green",
										});
										frm.reload_doc();
									}
	...
	// FIX: removed async:false (deprecated synchronous XHR).
	// Settings are fetched asynchronously and cached after first call.
	duration(frm) {
		if (!frm.doc.duration) return;

		const check = (settings) => {
			if (!settings || !settings.enforce_duration_limit) return;

			const max_dur = settings.max_duration_minutes || 120;
			if (frm.doc.duration <= max_dur) return;

			const msg = `Duration (${frm.doc.duration} mins) exceeds the maximum of ${max_dur} mins.`;
			...
			if (settings.duration_enforcement_mode === "Hard") {
				...
				frappe.show_alert(
					{ message: __(msg + " Consider shortening the meeting."), indicator: "orange" },
					7
				);
			}
		};

		if (_huddleSettingsCache) {
			check(_huddleSettingsCache);
		} else {
			frappe.call({
				...
				args: {
					...
					fieldname: [
						...
						"duration_enforcement_mode",
					],
				},
				callback(r) {
					if (r && r.message) {
						_huddleSettingsCache = r.message;
						check(r.message);
					}
				},
			});
	...
	status(frm) {
		if (frm.doc.status === "Cancelled" && !frm.__is_cancelling) {
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `
							<p>${__("You are about to cancel this huddle. Do you want to reschedule it instead?")}</p>
			...
			d.$wrapper.find(".btn-reschedule-huddle").click(() => {
				...
				frappe.msgprint(
					__("Please update the <b>Meeting Date</b> and save to notify participants.")
				);
